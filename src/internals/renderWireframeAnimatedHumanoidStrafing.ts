...
export function renderWireframeAnimatedHumanoidStrafing(
  scene: THREE.Scene,
  ...
  boxStackRenderer.asSceneObject().position.set(15, -15, 0);
  wireframeStackRenderer.asSceneObject().position.set(15, -15, 0);

  // boxStackRenderer.asSceneObject().rotateZ(-Math.PI * 0.25);
  // wireframeStackRenderer.asSceneObject().rotateZ(-Math.PI * 0.25);
    ...
    { // render strafing legs
      ...
      const debugLegAnimation = (
        ...
        const allWaypoints: glm.ReadonlyVec3[] = [
          [mainOrigin[0]+0.5, mainOrigin[1] + 0.5 * coefY, mainOrigin[2]-9 + 1.5*0],
          [mainOrigin[0]+0.5, mainOrigin[1] - 1.5 * coefY, mainOrigin[2]-9 + 1.5*1],
          [mainOrigin[0]+0.5, mainOrigin[1] - 3.5 * coefY, mainOrigin[2]-9 + 1.5*0],
        ];
