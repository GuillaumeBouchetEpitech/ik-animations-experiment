...
import {ICircle, circleCircleIntersectionPoints} from "./circleCircleIntersectionPoints"
...
export class LimbData {
  ...
  computeIk_fixedYaw(
    ...
    const invRootMat4: glm.ReadonlyMat4 = glm.mat4.invert(glm.mat4.create(), this.rootMat4);
    const rawLocalTarget: glm.ReadonlyVec3 = glm.vec3.transformMat4(glm.vec3.create(), inWorldTarget, invRootMat4);

    const rawLocalForward: glm.ReadonlyVec3 = glm.vec3.transformMat4(glm.vec3.create(), inWorldForward, invRootMat4);

    const primaryYaw = Math.atan2(rawLocalForward[1], rawLocalForward[0]);

    const tmpYawAlignedMat4 = glm.mat4.identity(glm.mat4.create());
    glm.mat4.rotate(tmpYawAlignedMat4, tmpYawAlignedMat4, primaryYaw, [0,0,1]);
    const tmpRollTarget: glm.ReadonlyVec3 = glm.vec3.transformMat4(glm.vec3.create(), rawLocalTarget, glm.mat4.invert(tmpYawAlignedMat4, tmpYawAlignedMat4));

    const primaryRoll = Math.atan2(tmpRollTarget[1], -tmpRollTarget[2]);

    const baseMat4 = glm.mat4.identity(glm.mat4.create());
    glm.mat4.rotate(baseMat4, baseMat4, primaryYaw, [0,0,1]);
    glm.mat4.rotate(baseMat4, baseMat4, primaryRoll, [1,0,0]);

    const localTarget = glm.vec3.transformMat4(glm.vec3.create(), rawLocalTarget, glm.mat4.invert(glm.mat4.create(), baseMat4));
  ...
  computeIk_fixedRoll(
    ...
    const rawLocalRoll: glm.ReadonlyVec3 = glm.vec3.transformMat3(glm.vec3.create(), inWorldRoll, glm.mat3.fromMat4(glm.mat3.create(), invRootMat4));

    const primaryRoll = Math.atan2(rawLocalRoll[1], rawLocalRoll[0]);

    const tmpRollAlignedMat4 = glm.mat4.identity(glm.mat4.create());
    glm.mat4.rotate(tmpRollAlignedMat4, tmpRollAlignedMat4, primaryRoll, [1,0,0]);
    const tmpThetaTarget: glm.ReadonlyVec3 = glm.vec3.transformMat4(glm.vec3.create(), rawLocalTarget, glm.mat4.invert(tmpRollAlignedMat4, tmpRollAlignedMat4));
  ...
  private _computeIk_joints(
    ...
    const circleB: ICircle = { center: [result.localTarget[0], result.localTarget[2]], radius: this.secondaryLength };
    ...
    const diffSecondaryA: glm.ReadonlyVec3 = glm.vec3.sub(glm.vec3.create(), result.localTarget, result.jointA.localPos);
    ...
    const diffSecondaryB: glm.ReadonlyVec3 = glm.vec3.sub(glm.vec3.create(), result.localTarget, result.jointB.localPos);
  ...
  extractBaseTransform(result: Readonly<ILimbDataResult>, outMat4: glm.mat4): void {
    glm.mat4.multiply(outMat4, this.rootMat4, result.baseMat4);
  }
  extractPrimaryTransform(result: Readonly<ILimbDataResult>, joint: Readonly<LimbDataResultJoint>, outMat4: glm.mat4): void {
    this.extractBaseTransform(result, outMat4);
    glm.mat4.rotate(outMat4, outMat4, joint.primaryPitch, [0,1,0]);
  }
  extractSecondaryTransform(result: Readonly<ILimbDataResult>, joint: Readonly<LimbDataResultJoint>, outMat4: glm.mat4): void {
    this.extractPrimaryTransform(result, joint, outMat4);
    glm.mat4.translate(outMat4, outMat4, [this.primaryLength,0,0]);
    glm.mat4.rotate(outMat4, outMat4, -joint.secondaryPitch - joint.primaryPitch, [0,1,0]);
  }
  extractTransforms(result: Readonly<ILimbDataResult>, joint: Readonly<LimbDataResultJoint>, baseMat4: glm.mat4, primaryMat4: glm.mat4, secondaryMat4: glm.mat4): void {
    this.extractBaseTransform(result, baseMat4);
    primaryMat4 = glm.mat4.rotate(primaryMat4, baseMat4, joint.primaryPitch, [0,1,0]);
    glm.mat4.translate(secondaryMat4, primaryMat4, [this.primaryLength,0,0]);
    glm.mat4.rotate(secondaryMat4, secondaryMat4, -joint.secondaryPitch - joint.primaryPitch, [0,1,0]);
  }
